...
Status do_encoding(EncodeInfo *encInfo)
{
  //Validations 
   if(validations(encInfo) == e_success)
   {
	...
	printf("INFO: Checking the %s size\n",encInfo->secret_fname);
	if(S_flag == 0)
	...
	{
	    printf("INFO: Failed to copy header file..!!\n");
	    return e_failure;
	}
	...
	{
	    printf("INFO: Failed to encode extension of secret file..!!\n");
	    return e_failure;
	}
	...
	{
	    printf("INFO: Failed copying remaining data\n");
	    return e_failure;
	}
   }
   else
